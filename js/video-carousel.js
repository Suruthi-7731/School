...
document.addEventListener('DOMContentLoaded', () => {
    ...
    style.textContent = `
        ...
        /* Hide default cursor on all carousel elements */
        .stories-container,
        .stories-scroll,
        .story-card,
        .unmute-btn {
        ...
        /* Show default cursor outside carousel */
        body *:not(.stories-container):not(.stories-container *):not(.book-cursor) {
    ...
    const observer = new IntersectionObserver((entries) => {
        if (autoScrolling) return; // Ignore observer changes during programmatic scroll
    ...
    videos.forEach((video, index) => {
        video.addEventListener('ended', () => {
            if (index < cards.length - 1) {
                ...
                // Loop back to start or just stop? Let's loop for "infinite" feel
                scrollToNext(0);
            }
        });
    });
    ...
    function scrollToNext(nextIndex) {
        ...
        // Use native smooth scrolling for testimonial slides only
        const targetScrollLeft = nextCard.offsetLeft - scrollContainer.offsetLeft;
        
        // Manually animate with requestAnimationFrame for smooth testimonial sliding
        const startScrollLeft = scrollContainer.scrollLeft;
        const distance = targetScrollLeft - startScrollLeft;
        const duration = window.innerWidth >= 1025 ? 600 : 800; // Faster for desktop
        ...
        function animateScroll(currentTime) {
            ...
            // Easing function for smooth testimonial animation
            const easeProgress = 1 - Math.pow(1 - progress, 3);
            
            const currentScroll = startScrollLeft + (distance * easeProgress);
